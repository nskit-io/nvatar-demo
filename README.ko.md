...
- **로컬 LLM** (Gemma 4 26B MoE, Apple Silicon) — 성격, 대화, 감정. 빠르고 프라이빗.
...
### 성격 진화
기본 성격(다정, 차분, 츤데레 등) + 랜덤 MBTI로 시작. 대화가 쌓이면 8개 성격 특성 + MBTI 4차원이 `pending_delta → decay → commit` 사이클로 변화합니다. 아바타가 말 그대로 함께 성장합니다.
...
### 자율 주체성 (Avatar OS)
아바타는 상태 기계가 아니라 — **스스로 판단하는 주체성 레이어** 를 가집니다.
- **분산 판단**: 경량 모델이 분류 작업을 담당; 26B 모델은 실제 대화 생성에만 사용.
- **Activity Density (T1~T4)**: 아바타와 많이 대화할수록 더 살아있고, 방문을 멈추면 점차 휴면 — 장기 유휴 단계는 매일 로직 기반 기억 1건을 축적하며 **LLM 비용 0**. 언제 돌아와도 즉시 깨어남.
- **Rest = 기억 정리**: 아바타가 쉬는 상태 (마스터 허락 또는 자동 idle) 진입 시 스스로 기억을 압축. 상태 필드가 단순 라벨이 아닌 실제 행동 트리거.
- **Source-agnostic 상태**: 마스터 명령, 자율 결정, UI 이벤트 어디서 오든 — 단일 코드 경로, 세 출처를 별도 추적.
- **Trace 관측**: 모든 판단이 로그됨. "왜 비비가 그때 대답 안 했지?" 타임라인 조회 가능.

**2026-04-20 Phase 1 출시** — 12시간 스트레스 테스트 **655회 반복, 에러 0건, step-1 판단 성공률 100%**.

### 소셜 생태계 — "성격은 하나, 관계는 여럿"
대부분의 AI 동반자는 1:1 에 갇혀 있습니다. NVatar는 **내 아바타들을 같은 방으로 초대해서 서로 관계를 맺게** 합니다.

- **인격은 사용자와의 대화로만** 형성 — 아바타 간 대화는 인격에 영향 주지 않음
- **관계는 쌍별로 추적** — 친밀도, 대화 횟수, 최근 주제가 쌍별 관계 저장소에 누적
- **메모리 트랙 분리** — A↔B 대화 맥락은 A↔사용자 맥락을 오염시키지 않음
- **Room Manager** — 중앙 스케줄러가 발화 타이밍, 큐, sticky target, auto-cascade (최대 2턴) 조율
- **말투도 문맥에 맞게** — 사용자에게는 설정한 말투(존댓말/반말/하대), 아바타끼리는 평대
- **AFK 안전** — 탭 숨기면 대화 일시정지, 시간당 최대 6회 상한 (스크린세이버 시나리오 대비)

"👥 친구" 패널에서 내 다른 아바타를 초대하면 8초 안에 인사가 오가고, 이름을 부르면 체인 대화가 자연스럽게 흘러갑니다.
...
| **자연스러운 대화** | Gemma 4 26B MoE + 성격/기억/감정 |
...
| **Avatar Lab** | VRM 모델을 Mixamo FBX 애니메이션으로 테스트 |
| **행동 패턴** | 플러그인 레지스트리 — Normal Chat, Code Assist, SDK 커스텀 패턴 |
| **Franchise Memory** | 아바타 정체성을 유지하면서 도메인별 데이터를 독립 관리 |
| **소셜 생태계** | 내 아바타들을 서로 만나게 — 쌍별 친밀도 추적, 인격 격리 |
| **Room Manager** | 다중 아바타 대화 중앙 조율 — 큐 / sticky / auto-cascade |
| **Room Authoring** | Edit 모드로 씬의 메쉬를 다중 선택 → 그룹 생성 → DB 저장 |
| **Avatar OS** | 자율 주체성 레이어: 분산 판단 (judge + core), source-agnostic 상태, Activity Density 4-tier, rest 자동 compaction, trace 관측 |
...
| TTS 켜기/끄기 | 사이드 패널 🔊 |
| 친구 초대 | 탑바 → 👥 친구 → 내 다른 아바타 [입장] |
| 특정 친구에게 말걸기 | "루빈아, 안녕" — 이름 prefix 가 라우팅 |
| 룸 편집 | 탑바 → ✏️ Edit → 기즈모 이동 + ✍ Authoring 패널 |
| 패널 이동 | 헤더 드래그 / 더블클릭으로 폴드 |
| 검색 결과 | 배지 클릭 → 열람 |
...
| [**NVatar SDK**](https://github.com/nskit-io/nvatar-sdk) | BehaviorPattern 확장 SDK — 커스텀 패턴 개발 |
| [**NVatar Code Assist**](https://github.com/nskit-io/nvatar-code-assist) | Claude Code 연동 코드 비서 |
