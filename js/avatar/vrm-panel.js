...
export async function buildVrmListPanel() {
  ...
  } catch(e) { panel.innerHTML = '<small style="color:#f87171;">로드 실패</small>'; }
}
...
export async function scanAllVrmBones() {
  ...
  try {
    ...
    for (let i = 0; i < vrms.length; i++) {
      ...
      try {
        const gltf = await vrmLoader.loadAsync(v.path);
        const vrm = gltf.userData.vrm;
        if (!vrm || !vrm.humanoid) { updateCardScore(v.path, -1, '?'); continue; }
        ...
        const getArmAngle = (side) => {
          ...
          upper.getWorldPosition(uPos); lower.getWorldPosition(lPos);
          const dir = new THREE.Vector3().subVectors(lPos, uPos).normalize();
          return Math.acos(Math.abs(dir.y)) * (180 / Math.PI);
        };

        const tPoseL = getArmAngle('left'), tPoseR = getArmAngle('right');
        ...
        const animL = getArmAngle('left'), animR = getArmAngle('right');

        let armScore = '?', detail = '';
        if (tPoseL !== null && animL !== null && tPoseR !== null && animR !== null) {
          const deltaL = Math.abs(animL - tPoseL), deltaR = Math.abs(animR - tPoseR);
          const avgDelta = (deltaL + deltaR) / 2;
          armScore = Math.min(100, Math.round((avgDelta / 30) * 100));
      ...
      } catch(e) { console.error(`[Scan] ${v.name}:`, e); updateCardScore(v.path, -1, 'ERR'); }
    }
    status.textContent = `완료! ${vrms.length}개 스캔`;
  } catch(e) { status.textContent = '스캔 실패: ' + e.message; }
  btn.disabled = false;
...
function updateCardScore(path, score, detail) {
  ...
  if (!rateEl) return;
  if (score < 0) { rateEl.textContent = '로드실패'; rateEl.style.color = '#f87171'; return; }
  if (score === '?') { rateEl.textContent = '측정불가'; rateEl.style.color = '#64748b'; return; }
...
function updateCardVerified(path, ok) {
  ...
  const badge = card.querySelector('.vrm-badge');
  if (cb) cb.checked = ok;
  if (badge) { badge.className = 'vrm-badge ' + (ok ? 'ok' : 'ng'); badge.textContent = ok ? 'OK' : 'NG'; }
}
